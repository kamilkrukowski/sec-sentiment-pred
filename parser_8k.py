...
from edgar import DocumentType
...
class Parser_8K(Parser):
    ...
    sections = list(section_names.keys())
    ...
    def get_sections(self, htmltext: str, return_types: bool = False,
        ...
        """
        doctext = self.clean_text(htmltext)
    ...
    def get_section_types(self, htmltext: str) -> List[str]:
        """
        Get a list of all section types present in document.
        ...
        out = []
        for section_type in Parser_8K.sections:
            if self.section_exists(doctext, section_type):
                out.append(section_type)
        return out
